...
# the 3 group members' public keys, in order
...
# The server's community and public key where we send the registration
...
# register the dataclasses as wire payloads with their message ids
...
class MyCommunity(Community):
    ...
    def __init__(self, settings: CommunitySettings) -> None:
        ...
        self._submitted = False   # guard so we only register once
        # poll for the server every 3s, first attempt after a 3s delay
        self.register_task("find_server", self.find_server, interval = 3.0, delay = 3.0)

    async def find_server(self) -> None:
        if self._submitted:
            return   # if already registered, nothing to do
        # look for the server among discovered peers by its known public key
        ...
        # send the 3 member keys to register the group
    ...
    def on_response(self, peer: Peer, payload: ResponseMessage) -> None:
        # print the server's reply; copy the group_id from here
        logging.info("Response: success=%s, group_id=%s, message=%s", payload.success, payload.group_id, payload.message)

async def start_communities() -> None:
    # build a single IPv8 overlay using our key file and start it
